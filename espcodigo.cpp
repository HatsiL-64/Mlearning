...
const char * leerUrl="https://unattested-unanswerably-ilona.ngrok-free.dev/procesar";
const char *reconocerUrl = "https://unattested-unanswerably-ilona.ngrok-free.dev/detectar";
...
const int PIN_BOTONA=12;
const int PIN_BOTONB=13;
//Declarar el flash
...
void enviarFoto(const char *modo)
{
  //Encender flash
  ...
  if(http.begin(client,modo))
  {
    //avisar que se manda
    http.addHeader("Content-Type","image/jpeg");
    //http.addHeader("Image-Id", modo);
...
void loop() {
  ...
  if(digitalRead(PIN_BOTONA)==LOW)
  {
    Serial.println("Boton A oprimido, enviando foto lectura");
    enviarFoto(leerUrl);
    delay(10);
    while(digitalRead(PIN_BOTONA)==LOW)
    {
      delay(10);
    }
  }
  if(digitalRead(PIN_BOTONB)==LOW)
  {
    Serial.println("Boton B presionado, enviando foto reconocimiento");
    enviarFoto(reconocerUrl);
    delay(10);
    while(digitalRead(PIN_BOTONB)==LOW)
    {
      delay(10);
    }
  }
